...
n_occasions <- 5 
# simulate ScrData 
scrdat <- simulate_scr(true_par, n_occasions, detectors, mesh, seed = 25939)
...
test_that("ScrModel checks inputs", {
  ...
  expect_error(ScrModel$new(form, scrdat$capthist(), start), "Must supply a ScrData object for data.")  
  expect_error(ScrModel$new(form[[1]], scrdat, start), "Invalid list of formulae.")
  expect_error(ScrModel$new(list("lambda0 ~ 1", "sigma ~ 1", "D ~ 1"), scrdat), "Invalid list of formulae.")
  expect_error(ScrModel$new(form, scrdat, c(0.5, 20, 300)), "start must be a list of starting values for each parameter.")
  expect_error(ScrModel$new(form, scrdat, start, print = "TRUE"), "print must be TRUE or FALSE.")
})

test_that("Basic model works" {
  mod <- suppressWarnings(ScrModel$new(form, scrdat, start))
  mod$fit()
...
test_that("Detection covariates model works" {
  ...
  mod <- suppressWarnings(ScrModel$new(formdetcov, scrdat, start))
  mod$fit()
  ...
  mod <- suppressWarnings(ScrModel$new(formtimecov, scrdat, start))
  mod$fit()
  ...
  mod <- suppressWarnings(ScrModel$new(formdettimecov, scrdat, start))
  mod$fit()
...
test_that("Detection covariate smooths work", {
  ...
  mod <- suppressWarnings(ScrModel$new(formsm, scrdat, start))
  mod$fit()
...
test_that("Density surface models work", {
  ...
  mod <- suppressWarnings(ScrModel$new(formd, scrdat, start))
  mod$fit()
  ...
  mod <- suppressWarnings(ScrModel$new(formsd, scrdat, start))
  mod$fit()
