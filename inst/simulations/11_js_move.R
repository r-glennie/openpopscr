...
# set truth 
true_par <- list(D = 1000, lambda0 = 0.5, sigma = 20, phi = 0.8, beta = 0.5, sd  = 10)
...
for (sim in 1:nsims) {
  ...
  obj <- JsTransientModel$new(form, scrdat, start, print = FALSE)
...
sum(lcl[4,] < b & ucl[4,] > b)
sum(lcl[5,] < log(true_par$sd) & ucl[5,] > log(true_par$sd))
sum(lcl[6,] < log(true_par$D) & ucl[6,] > log(true_par$D))
