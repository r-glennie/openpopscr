...
In this vignette, I describe how spatial capture-recapture models can be used within an <code>R6</code> object of the <code>ScrModel</code> class. The folder inst/examples/ in the [repository](https://github.com/r-glennie/openpopscr) contains example code.  

```{r load_pkg, eval = TRUE}
library(openpopscr)
library(secr)
```
```{r secretload, include = FALSE, eval = TRUE}
...
When adding the covariate to the data, I specify the covariate is of type "j" because it is a detector-level covariate. See the vignette on the ScrData class for
further information. 
...
```{r spacemod, results = "hide", cache = TRUE}
form <- list(lambda0 ~ 1, 
             sigma ~ 1, 
             D ~ x*y)
...
mod_D2$fit()
```
```{r spacemod2, eval = TRUE}
mod_D
```

Models can be compared by AIC: 
```{r spaceaic, eval = TRUE}
AIC(mod_D, mod_D2)
```
...
start <- get_start_values(scrtransdat)
```
```{r statscrmake, results = "hide", cache = TRUE}
# create model object 
stat <- ScrModel$new(stat_form, scrtransdat, start)
# fit model 
stat$fit()
```
```{r analysestat, eval = TRUE}
# look at results 
stat
...
the <code>get_start_values</code> function so that a start value for the movement
