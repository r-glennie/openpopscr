...
The model is fit usual way and similar output obtained. 
```{r, eval = FALSE}
impact_mod$fit()
```
```{r, echo = FALSE}
impact_mod <- readRDS("fitted_mod_impact.Rds") 
```
```{r}
impact_mod 
```
...
```
```{r, echo = FALSE}
t_mod <- readRDS("fitted_mod_beta.Rds") 
```
```{r}
t_mod 
```
...
The samples obtained can be used for model checking or for Monte Carlo testing 
